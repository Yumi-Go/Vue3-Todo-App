...
import { useBookmark, useUnBookmark } from '../composables/bookmark'
...
<v-container fluid class="flex flex-col justify-center items-center px-2 py-0">
    <v-list bg-color="pink" class="w-full h-[500px] text-white">
        ...
        class="hover:bg-red-300 hover:text-black group">
            <v-list-item-action>
                <v-checkbox v-model="task.completed">
                    <template #label>
                        <span class="font-bold" :class="{ 'line-through': task.completed }">
                            {{ task.name }}
                        </span>
                    </template>
                </v-checkbox>
